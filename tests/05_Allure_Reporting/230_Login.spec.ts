...
test('Verify that the login works', async({page}) =>{
    ...
    await allure.description("Verify that the login works");
...
// You’re using PowerShell. `export` is bash syntax. 
// In PowerShell use: $env:ALLURE_LABEL_epic = “WebInterface”. 
// For allure serve, install via: npm i -g allure-commandline, 
// then run: npx allure serve allure-results.

// to generate HTML report - allure generate